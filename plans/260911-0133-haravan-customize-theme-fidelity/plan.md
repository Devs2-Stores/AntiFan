...
| Session rotation | **not eliminated** — the superseded session is ended after the mint, and a 2-pair diag is clean (exit 0, `released: true`, secret never recorded), but the 4-pair fixture refused 2 of 4 legs and the post-anchor full run lost 6 of 21 (`404@390`, `collection@1024`, `product@1024`, `product@390`, `search@1440`, `search@390`) against 11 of 42 before the release existed. Each failure is a `read ECONNRESET` inside `canary-session.mjs`'s own pairing socket, not a quota refusal: the port's 10-tab limit is per bound session (`src/main/tools/browser-control-port.ts:2071-2079`), so a superseded session cannot block a fresh mint. Type it as an unproven bridge/transport defect correlated with teardown. The adjacency experiment is **implemented** (`theme-fidelity.mjs:1240-1253`) and kept for its failure handling — a failed mint no longer strands the run's tab — not as a demonstrated remedy: the class is **not eliminated**, the failure moved legs (before: `cart__1440x900` and `home__1440x900` refused while `article__1024x900` measured; after: `cart__1440x900` measured with `released: true` while `article__1024x900` refused), same count on the legs observed, no reduction, on a fixture whose base session came from an aborted compare. Still open: a settle gap between mint completion and the tab close, a bounded transport-only mint retry, per-pair lifecycle isolation; `git revert 874060e` is the cheap exit if the reorder proves inert beyond its failure handling |
