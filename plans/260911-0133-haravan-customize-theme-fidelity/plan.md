...
| Settle instrument | improved, **not closed** — the scroll offset is anchored to a fixed value before the read block and the image settle requires identity plus document geometry rather than completeness alone; neither is an input to `decideSettle`, whose fields and tolerances are unchanged. The 9 s deadline path returns `imagesSettled: pendingImages === 0 && stableSamples >= 2` (about 200 ms of stability), so a deadline-path `imagesSettled: true` is a floor, not proof |
| Session rotation | **cleared at fixture scale, not at campaign scale** — the superseded session is ended after the mint, and `renewSession` additionally re-runs the mint up to 3 times with a 3 s gap (transport only, a measurement is never replayed), recording each attempt as `sessionRenewal` and keeping the typed refusal when all attempts fail. Two post-retry 4-pair runs refused 0 of 4 legs, with `released: true` on all four, and needed it on two of them (attempts across the twelve fixture leg-runs: article 2/1/1, cart 2/3/1, home 1/1/2, product 1/2/1; five of twelve needed more than one attempt, `p = 0.417`). The residual is an upper bound, not a rate: those twelve leg-runs are four legs repeated three times, and the hazard falls with each attempt (attempt 1 5/12, attempt 2 1/5, attempt 3 0/1), so a constant-hazard `p^3` of about 1.5 of 21 legs over-states it and carries no point estimate. `MINT_ATTEMPTS` stays at 3 — raising it would rest on a memoryless model the data refutes and would mix a second variable into the next campaign run, and a bound change needs its own measurement. The pre-retry 4-pair fixture refused 2 of 4 legs and the post-anchor full run lost 6 of 21 (`404@390`, `collection@1024`, `product@1024`, `product@390`, `search@1440`, `search@390`) against 11 of 42 before the release existed. Each failure is a `read ECONNRESET` inside `canary-session.mjs`'s own pairing socket, not a quota refusal: the port's 10-tab limit is per bound session (`src/main/tools/browser-control-port.ts:2071-2079`), so a superseded session cannot block a fresh mint. Type it as an unproven bridge/transport defect correlated with teardown, not a quota refusal. The adjacency reorder is **implemented** (`theme-fidelity.mjs:1240-1289`) and kept for its failure handling — a failed mint no longer strands the run's tab — not as a demonstrated remedy: alone it only moved the failing leg. Publication is gated by the other classes now: the same run refused `article__1024x900` and `home__1440x900` on `content-changed-between-passes`, and `cart__1440x900` on `REFERENCE_IDENTITY_DRIFT+SUBJECT_IDENTITY_DRIFT`. Untried lever: a settle gap between mint completion and the tab close |
| Image hydration | fixed — the storefront's loader parks a base64 placeholder in `src`, so the previous promotion guard never fired; the loader's own contract is now applied to both sides. The residual image-identity move on `article__1024x900`/`home__1440x900` is measured as two distinct mechanisms, neither of them plain loading: the document holds (`docHeight` 4821/5426–5428, `scrollWidth` 1024/1425), the page is at `scrollY` 0 on every pass, `pendingImages`/`brokenImages` are 0, and the within-pass identity diff is empty. Two positional movers appear once, between pass 1 and pass 2, and hold through pass 3 — the second `logo-hlt-2024.png` instance (`y` 30 to 52/55) and a 300x150 image named `%3E` (`y` 795 to 4675/5321); a rect cannot separate a re-render from a resumed carousel transform, so neither is asserted. The repeated mechanism is the `imageSetHash` move, which the panel cannot see because `panel`/`identityCounts` key on the last path segment while the hash uses the full `currentSrc` plus natural size — so **the hash mover is still unnamed**, and the panel cannot see it for a second reason recorded below. The one-shot promotion is also re-swapped: `IMAGE_HYDRATION_EXPR` runs at `theme-fidelity.mjs:1583`, before `hydrateToCapturedState`, and the replay executes the page's own inline lazysizes, while no uncovered loader selector exists in the served corpus (0 `data-srcset`, 0 `data-bg`, 0 `<source>`, 0 `img[srcset]` across all eight dumps; the 104 `data-original` hits are on non-`img` elements), so the mover is not a `src` assignment either: `imageSetHash` hashes `currentSrc|naturalWxH|rect(x, y+scrollY, w, h)|complete` per image in DOM order (`canary-settle.mjs:155-162`, hash at `:173`), and an index-aligned diff shows pass 2 to pass 3 identical in `src`, natural size, `y` and DOM order — so `x`, the rendered `w`/`h`, or `complete` moves it, and node re-ordering is excluded on both intervals. The next evidence fields are the full rendered rect, `complete` and the DOM index per pass plus a per-pass loader-selector count, and the lever is re-applying the promotion before each measurement read. The `lazyloaded` normalisation was tested inside that promotion, changed no verdict and no mechanism, and was reverted; the full `imagePanel` and the `imageIdentityChanges` list stay as evidence-only fields. In a 4-pair fixture `product__1440x900` went `content-changed-between-passes` to `PASS/MATCH` and `article__1024x900` stopped drifting (docHeight constant 4821, scrollWidth constant 1024) while still swapping image identity |
...
`status` stays `blocked`: publication waits on `260911-0652` Phase 1 (route assertion — in flight uncommitted
in the tree at handover), on the image-identity class or the owner's typed-refusal decision, and on the r1
reference asymmetry. The session-renewal transport defect now costs a bounded retry instead of a leg at
fixture scale, and remains to be re-measured at 21 legs. A campaign re-run before the first two land returns
`INCOMPLETE` again. No run is in flight.
