...
`status` stays `blocked`: publication waits on `260911-0652` Phase 1 (route assertion), on the session-renewal
transport defect (`read ECONNRESET` in the mint, not a quota refusal), and on the image-identity class or the
owner's typed-refusal decision. A campaign re-run before those land returns `INCOMPLETE` again — the renewal
class alone keeps both compare sets from completing. No run is in flight.
