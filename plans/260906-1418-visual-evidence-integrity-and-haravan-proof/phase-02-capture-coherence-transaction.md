...
- [x] R1. `TwoSourceCoherenceGuard` TRONG `visual-capture.ts` (3-module cap cố định:
  ...
  (không so sánh tiếp).
- [x] R2. Joint resource acquisition: dùng mutex khoá theo key (FIFO per-key promise
...
- [x] R3. Coherence trả về receipt `identityCoherent`, và kết quả visualCompare bổ
  sung trường `coherence`/`captureStateCompatible` (mở đường Phase 3).
- [x] R4. Không đổi public IPC/MCP input schema của `browser.visual_compare`.
...
- [x] `TwoSourceCoherenceGuard` pure implementation
- [x] Host identity APIs trả về per-side (documentGeneration, browserEpoch, mutationRevision)
- [x] Pre/post check trong visualCompare (RESAMPLE / TARGET_STALE)
- [x] Asymmetric normalization → INCONCLUSIVE (hủy diff)
- [x] `MultiKeyLock` (keyed FIFO, sorted multi-key acquire, release trong finally)
- [x] Unit tests lock: serial hơn trên cùng key, song song trên key khác, không deadlock
- [x] Joint lock: `passivePool` (capacity) + `MultiKeyLock` (loại trừ) phối hợp
- [x] Tier 2 tests: V-13, V-14, V-15
