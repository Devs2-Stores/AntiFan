...
- Propagate `AuthenticatedCapabilityContext.signal` through every browser capability handler and `BrowserControlPort` action/wait method into `ViewportGate`/wait-registry ownership. A cancelled queued action is removed before acquisition and cannot execute later as an orphan.
...
- [ ] Every browser capability and control-port action forwards the Main-owned signal to its exact scheduler owner; aborted queued work is removed and never dispatches.
...
| Trusted CDP fails after partial dispatch | Synthetic fallback could duplicate or complete an uncertain action | Permit fallback only with proven zero input; otherwise persist `unknown`/`interrupted` and stop. |
