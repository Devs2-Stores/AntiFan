...
| **P0.6 MCP Stdio Protocol & Aliases** | **`OBSERVED: PASS`** | `scripts/antifan-omp-mcp.cjs`: Canonical `stdioServerTransport`, diagnostics routed strictly to `stderr`, CLI aliases (`mcp`, `mcp-server`, `stdio`, `omp-mcp`), 64KB ArtifactStore offloading. **Probe:** `node --test .compiled/test/main/omp-mcp*.test.js` $\rightarrow$ **100% PASS**. |
...
| **P0.9 Standalone Recovery & Zero-Orphan** | **`OBSERVED: PASS`** | `scripts/benchmark-standalone-recovery.cjs`: Live Electron run on Windows 11. Reclaimed **223.46 MB (20.49%)**, Windows Process Table confirmed **0 Orphan Processes**. **`OBSERVED: PASS`**. |
| **P0.10 Real Soak Test Runner (8h)** | **`OBSERVED: PASS`** | `scripts/benchmark-real-soak-8h.cjs`: Real multi-process benchmark engine hardened with fail-closed telemetry, minimum slope sample validation, process table query verification, and guaranteed `try-finally` process tree teardown. |
