...
| U31 | TEST | MED | CONFIRMED (repaired after a third failure) | `goal-runner.test.mjs` "loses at most one unit of work when killed mid-phase" waited on progress with `waitFor`'s 15 s default (`test/unit/goal-runner.test.mjs:41,105`). The assertion is right; the budget was environment-fragile - it failed at **15.03 s** in one `test:fast` run, passed in 3.98 s in the next, and failed again at **15.08 s** in the first full `npm test -- --verify` run while fifteen other lanes ran clean. The case itself needs only the first two fixture units (6 x 400 ms) plus runner startup: 3.4 s standalone, 4.2/3.7 s in two isolated `test:fast` runs. | repaired - the helper's default budget is now 60 s (`test/unit/goal-runner.test.mjs:41`), so the wait times the runner's progress rather than the host's scheduling, with the measured numbers in the comment. The stall assertion is unchanged: a runner that stops progressing still fails it, and the failure message now prints the worklog start count and the checkpoint verdicts so a recurrence explains itself. Explicit per-case budgets below are untouched. |
