...
| U31 | TEST | MED | CONFIRMED (repaired - waits are now progress-based) | `goal-runner.test.mjs` "loses at most one unit of work when killed mid-phase" waited on the runner with a *calendar* budget, so it timed the machine: fail at **15.03 s**, pass at 3.98 s, fail at **15.08 s**, and after the budget was raised to 60 s, fail at **60.08 s** with `entries=5 verdicts=[PASS,pending x5]` on an idle-host run of 3.4-4.5 s. The assertion was right; the bound measured the host. | repaired - `waitForProgress` (`test/unit/goal-runner.test.mjs`) bounds a **stall** (no new worklog entry for 16 s = 40x the fixture's 400 ms unit), so a loaded host only moves the wall clock while a runner that stops fails in 16 s with `describeRun()` evidence: last worklog entries, heartbeat age, lock liveness. The resumed runner's `exit` wait is bounded at 120 s instead of unbounded. Negative control (throwaway copy of the case pointed at a never-resolving fixture, deleted): failed at **16.4 s** naming `no worklog entry for 16021ms ... heartbeatAgeMs=16037 runnerAlive=true last=[run-start@..., unit-start:u1@...]`. `test:fast` green twice after the repair (1261/0; the case at 4.47 s and 3.84 s). |
| U32 | PRODUCT | MED | CONFIRMED | Renewal accumulates lease time instead of sliding it: `renewAttachment` sets `expiresAt = Math.max(record.expiresAt, now) + extensionMs` (`src/main/run/attachment-registry.ts:1047`) and mirrors the same runaway into `record.lease.expiresAt` (`:1048`). Because a deadline that exists is always in the future, every renewal adds a full extension instead of restarting the window: a client renewing on the bridge's 30 s heartbeat (`BRIDGE_HEARTBEAT_INTERVAL_MS`, `src/main/bridge/bridge-server.ts:145`) with the default 1 h extension (`:1046`) grows the deadline by 1 h per tick, so a day of heartbeats leaves roughly 120 days of remaining TTL. The field is load-bearing for liveness, not decoration: the dead-target gates at `:602`/`:722`, `validateAttachment` `:958`, the renew guard `:980` and the retention sweeps `:254`/`:1147` all read `expiresAt`. Pre-existing at HEAD (core-freeze commit `8a0d893c`); the U2 persist throttle is the only change in this method, so this is a new finding, not a regression from that repair. | deferred - accumulate vs slide is a product decision (may an early heartbeat push the deadline past `now + extensionMs`?); the repair is a one-line shift to `now + extensionMs` (same for `lease.expiresAt`) plus a test that two renewals inside one window do not stack. Not repaired here: live callers depend on the current contract and no lane exercises the multi-renewal case. |
| U33 | HARNESS (divergence) | Wiring the terminal lanes into the pipeline (`f67c2bf3`) made both red on their first run: they created their `BrowserWindow` without `sandbox: false`, which production sets on every standalone view (`src/main/browser/native-tab-host.ts:949,970,988,8034,8134`). Electron's default sandbox cannot `require` a relative module, so `standalone-preload.js` never loaded (`Unable to load preload script` + `module not found: ../shared/contracts` from the sandbox bundle), the renderer had no bridge, and GATE-B failed with `views: []`. Repaired by mirroring the production option. Remaining divergence in the same class, not repaired: the renderer calls main channels the lanes never register - `antifan:terminal:get-all-affinities` (2 calls in each lane) plus, in the rename lane, `antifan:terminal:get-full-buffer` (2), `antifan:terminal:sync-view` (2), `antifan:terminal:resize-session` (2) and `antifan:tabs:get-list` (1); Electron prints "No handler registered" for each and no assertion covers it. | sandbox divergence fixed (both lanes green, transport receipt `[P0-Transport Certified]` with all six gate records); handler divergence `deferred` - registering the channels with production semantics starts exercising sync/buffer/affinity paths, a scope decision rather than a silent patch. |
| U34 | PIPELINE (observability) | In the first `npm test -- --verify --json` run, `test:terminal-transport` failed but its final block (the GATE-B `AssertionError`) never reached the captured stdout, while the same lane run directly prints it; the runner uses `stdio: 'inherit'` for every lane. The verdict stayed recoverable because the lane writes `plans/reports/terminal-p0-transport-certification.json` with `verdict: FAILED` and the error string even on the failure path. Cause not isolated at capture time (layered npm/cmd handles on Windows is the hypothesis, unverified). | info - diagnosability only: the durable receipt carries the verdict and message, so the run is never silent about why a lane failed. |
| U35 | TEST (unexplained stop) | The runner behind U31 stopped advancing **inside** a pipeline run: `entries=5`, one verdict, 60.08 s, while the same case finishes in 3.4-4.5 s. The cause was not captured and three reproduction attempts did not stall: the full 6-unit ladder standalone 2.5 s, four runners in parallel 5 s, and the case inside `test:fast` 4 s. Candidates the old evidence cannot separate: a blocking durability write (`fsyncSync`/`renameSync` inside `writeRecordAtomic`) or the process dying - neither liveness nor heartbeat age was recorded at the time. | info - no product change until it is observed again: `describeRun()` now reports the last worklog entries, heartbeat age and lock liveness on failure, which names the culprit on the next occurrence. |
| U36 | PRODUCT (same invariant as U30) | The autoheal path set only `process.env.ANTIFAN_BOUND_TAB_ID = session.tabId` (`scripts/antifan-omp-mcp.cjs:1977`) while `resolveBoundTabId` (`:164`) reads the observed override first - so an override recorded by an earlier switch/rebind/open outlived a foreign-instance failover and kept naming a tab on the instance the proxy had just left; every omitted-tabId call then rode it into `TARGET_MISMATCH`. Reachable: failover candidates include locally discovered instances (`resolveFailoverCandidates`, `:239`) when the terminal-instance context env is set. | fixed - the site calls `recordBoundTab(session.tabId)`, the module's single owner, which moves the override and the env together. The existing rotation test covers the switch/rebind/open/close seams; the autoheal seam itself has no case (a gap, not a pass). |
| U37 | PRODUCT | MED | CONFIRMED (deferred) | `src/main/session/issue-register.ts` `rewriteFile()` (`:1149`) merges the disk file with the in-memory `this.issues` loaded at construction, and issue reads never re-read the file (unlike verifications, which read fresh per mutation): a process holding a stale record that rewrites any other issue writes its stale copy of the untouched id over a peer's newer write. Not introduced by this range; reachable only when two processes share one data root. | deferred - the fix changes merge semantics inside a persistence layer and needs a dedicated multi-process test; the range's register work is durability, not concurrency. Recorded, not touched. |
| U38 | INFO (review pass, recorded) | Smaller observations from the pre-push review, none a regression: `tab-devtools-host.ts:2126` re-labels a real `CAPTURE_TIMEOUT` as `NO_RENDER_SURFACE` when `capturePage` hangs past its 4 s bound on an attached view (timed-out capture promises also accumulate); `issue-register.ts:951` dropped the 1000-record cap, so the register file grows with every rewrite (O(file) per write); `antifan-omp-mcp.cjs:2245` compares the close rotation against the invoke-head snapshot, so a racing switch-tab can clear a newer default (degrades to riding the authority, not a dead tab); `browser-capabilities.ts:385` lets an explicit `outputPath='.'` through as a bare EISDIR; `scripts/smoke-trusted-cdp.cjs:20` is the only Electron lane that does not pin `ANTIFAN_DATA_ROOT` (it does not touch the register, so no observed pollution); `scripts/verify-clone-parity.cjs:903` `--probe-sliders` forces exit 0 and is wired into no lane. | info - candidates for the next pass; none blocks the push. |
...
11. **U32** - renewal deadline accumulates, so the attachment TTL is unbounded
    under a repeating heartbeat; `deferred` on a product call about renewal
    semantics, not on any lane.
