...
import { canonicalizeWorkspaceRoot } from '../../shared/control-plane-contracts';
...
export function validateResultV2(
  data: unknown,
  expectedCommand?: { id: string; targetWorkspace: { folderUri: string }; promptDigest?: string; projectId?: string; workspaceId?: string; attemptId?: string; hostInstanceId?: string; hostEpoch?: number; backendSessionRef?: string }
): { ok: boolean; error?: string; result?: AntigravityResultV2 } {
  if (!data || typeof data !== 'object') {
  ...
  // Strict Request Binding Invariants
  if (expectedCommand) {
    if (res.commandId !== expectedCommand.id) {
      return { ok: false, error: `Receipt commandId mismatch: expected ${expectedCommand.id}, got ${res.commandId}` };
    }
    if (!res.targetWorkspace || typeof res.targetWorkspace.folderUri !== 'string') {
      return { ok: false, error: 'Receipt is missing targetWorkspace.folderUri' };
    }
    if (res.targetWorkspace && res.targetWorkspace.folderUri) {
      const resWs = canonicalizeWorkspaceRoot(res.targetWorkspace.folderUri);
      const expWs = canonicalizeWorkspaceRoot(expectedCommand.targetWorkspace.folderUri);
      if (resWs !== expWs) {
        return { ok: false, error: `Receipt targetWorkspace mismatch: expected ${expWs}, got ${resWs}` };
      }
    }
    const resDigest = res.promptDigest;
    if (expectedCommand.promptDigest && resDigest !== expectedCommand.promptDigest) {
      return { ok: false, error: 'Receipt promptDigest does not match request promptDigest' };
    }
    if (expectedCommand.projectId !== undefined && res.projectId !== expectedCommand.projectId) return { ok: false, error: 'Receipt projectId does not match request' };
    if (expectedCommand.workspaceId !== undefined && res.workspaceId !== expectedCommand.workspaceId) return { ok: false, error: 'Receipt workspaceId does not match request' };
    if (expectedCommand.attemptId !== undefined && res.attemptId !== expectedCommand.attemptId) return { ok: false, error: 'Receipt attemptId does not match request' };
    if (expectedCommand.hostInstanceId !== undefined && res.hostInstanceId !== expectedCommand.hostInstanceId) return { ok: false, error: 'Receipt hostInstanceId does not match request' };
    if (expectedCommand.hostEpoch !== undefined && res.hostEpoch !== expectedCommand.hostEpoch) return { ok: false, error: 'Receipt hostEpoch does not match request' };
    if (expectedCommand.backendSessionRef !== undefined && res.backendSessionRef !== expectedCommand.backendSessionRef) return { ok: false, error: 'Receipt backendSessionRef does not match request' };
  }
...
export class AntigravityCommandClient {
  ...
  private readonly lateReceiptGraceMs: number;
  ...
  constructor(options: AntigravityCommandClientOptions) {
    ...
    this.lateReceiptGraceMs = Math.max(0, options.lateReceiptGraceMs ?? 4500);
  ...
  dispatchCommand(params: DispatchCommandParams): {
    ...
    const command: AntigravityCommandV2 = {
      ...
      backendSessionRef: params.backendSessionRef,
    ...
    const resultPromise = this.pollForResult(command, timeoutMs);
    return { command, resultPromise };
  }

  private pollForResult(command: AntigravityCommandV2, timeoutMs: number): Promise<AntigravityResultV2> {
    const commandId = command.id;
    const bridgeDir = this.getBridgeDir();
    const resultFile = path.join(bridgeDir, `${commandId}.res.json`);
    const startTime = this.clock();
    let observedUnknown: AntigravityResultV2 | undefined;
    let lateReceiptStartedAt: number | undefined;
      ...
      const checkResult = () => {
        if (this.fs.existsSync(resultFile)) {
          try {
            ...
            const parsed = JSON.parse(raw);
            const val = validateResultV2(parsed, this.expectedReceiptBinding(command));
            if (val.ok && val.result) {
              if (isLateReconciliationCandidate(val.result)) {
                observedUnknown = val.result;
                lateReceiptStartedAt ??= this.clock();
              } else {
                this.cleanupPoller(command.id);
                try { this.fs.unlinkSync(resultFile); } catch {}
                resolve(val.result);
                return;
              }
            }
        ...
        if (elapsed >= timeoutMs && lateReceiptStartedAt === undefined) lateReceiptStartedAt = now;
        if (lateReceiptStartedAt !== undefined && now - lateReceiptStartedAt >= this.lateReceiptGraceMs) {
          this.cleanupPoller(commandId);
          try { this.fs.unlinkSync(resultFile); } catch {}
          resolve(observedUnknown || {
            protocolVersion: 2,
            commandId,
        ...
        this.activePollers.set(command.id, timer);
      };
  ...
  private expectedReceiptBinding(command: AntigravityCommandV2): { id: string; targetWorkspace: { folderUri: string }; promptDigest?: string; projectId?: string; workspaceId?: string; attemptId?: string; hostInstanceId?: string; hostEpoch?: number; backendSessionRef?: string } {
    const meta = command.meta || {};
    const strict = typeof meta.projectId === 'string' && typeof meta.workspaceId === 'string' && typeof meta.attemptId === 'string';
    return {
      id: command.id,
      targetWorkspace: command.targetWorkspace,
      promptDigest: strict ? command.promptDigest : undefined,
      projectId: strict ? meta.projectId as string : undefined,
      workspaceId: strict ? meta.workspaceId as string : undefined,
      attemptId: strict ? meta.attemptId as string : undefined,
      hostInstanceId: strict && typeof meta.hostInstanceId === 'string' ? meta.hostInstanceId : undefined,
      hostEpoch: strict && typeof meta.hostEpoch === 'number' ? meta.hostEpoch : undefined,
      backendSessionRef: strict && typeof command.backendSessionRef === 'string' ? command.backendSessionRef : undefined,
    };
  }
  ...
  checkLateReceipt(commandId: string, expectedCommand?: { id: string; targetWorkspace: { folderUri: string }; promptDigest?: string; projectId?: string; workspaceId?: string; attemptId?: string; hostInstanceId?: string; hostEpoch?: number; backendSessionRef?: string }): AntigravityResultV2 | null {
    const bridgeDir = this.getBridgeDir();
    const resultFile = path.join(bridgeDir, `${commandId}.res.json`);
    if (!this.fs.existsSync(resultFile)) {
      return null;
    ...
    try {
      const raw = this.fs.readFileSync(resultFile, 'utf8');
      const parsed = JSON.parse(raw);
      const val = validateResultV2(parsed, expectedCommand);
      if (val.ok && val.result) {
        if (val.result.deliveryState === 'unknown' && (
          val.result.errorCode === 'EXECUTION_DEADLINE_EXCEEDED' ||
          val.result.errorCode === 'SIDECAR_EXECUTION_TIMEOUT' ||
          val.result.errorCode === 'TIMEOUT_WAITING_RECEIPT'
        )) return null;
        try {
  ...
  scanAndReconcileLateReceipts(pendingCommandIds: string[], expectedCommands?: Map<string, { id: string; targetWorkspace: { folderUri: string }; promptDigest?: string; projectId?: string; workspaceId?: string; attemptId?: string; hostInstanceId?: string; hostEpoch?: number; backendSessionRef?: string }>): Map<string, AntigravityResultV2> {
    const reconciled = new Map<string, AntigravityResultV2>();
    for (const cmdId of pendingCommandIds) {
      const res = this.checkLateReceipt(cmdId, expectedCommands?.get(cmdId));
      if (res) {
...
function isLateReconciliationCandidate(result: AntigravityResultV2): boolean {
  return result.deliveryState === 'unknown' && (
    result.errorCode === 'EXECUTION_DEADLINE_EXCEEDED' ||
    result.errorCode === 'SIDECAR_EXECUTION_TIMEOUT' ||
    result.errorCode === 'TIMEOUT_WAITING_RECEIPT'
  );
}
