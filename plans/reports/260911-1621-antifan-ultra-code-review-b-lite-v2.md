...
| U6b | Important | `.canary/tools/fix-loop/merge-gate.mjs:224` | The same bypass survived **at the merge** — the point where the write actually happens — because `mergeStagedWorkspace`'s own audit called `runAllAudits` without `changedBytesFor`, so the merge priced a rewrite from the size delta while the standalone audit priced it from content. A file could be refused by `audit` and merged anyway. An unreadable or NUL-bearing rewrite additionally fell back to the net delta, i.e. it chose its own price. | pre-fix modules (`9d2922c`) run end-to-end against a staged fixture: `decision=OK budgets.bytes=1` with `maxBytes: 20`, and **the target was mutated**; post-fix: `REFUSED_DIFF_BUDGET`, target untouched. NUL-bearing 25 B rewrite: post-fix charged 25 B, ceiling 20 → refused. The pre-fix decision is also reproducible in-suite: the same manifests through the bare `runAllAudits` call (every argument the merge passed, no resolver) return `OK budgets.bytes=1`, measured in the same test as the refusal, so the control needs no module copies to be re-checked | in-range | **Fixed**: one module-level `changedBytesResolver` passed at both call sites; content that cannot be compared is charged its whole larger side, never nothing + 2 regression tests in the merge/audit pair |
...
| Gate | Result |
|---|---|
| `.canary/tools/fix-loop/test-fix-loop.mjs` | **21/21** (16 before; +defaults control, +`measureChangedBytes` unit, +staged byte-volume control, +merge-path pricing control with its no-resolver contrast pair, +unreadable-content control; self-verification test extended) |
