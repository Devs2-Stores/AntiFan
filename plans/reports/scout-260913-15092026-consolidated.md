...
## 2. Bản pull `15092026/` — khớp khoá 100%; nội dung đối chiếu theo `value`/`attachment` của API (không theo byte CDN, không theo `size`)

- 241 tệp / 241 khoá remote: **độ phủ khoá** 241/241, thiếu 0, thừa 0. Đối chiếu **nội dung** (theo `value`/`attachment`, cửa sổ 18:18:40Z–18:19:24Z): **237/237 khoá so sánh được đều khớp**, 4 khoá remote không trả nội dung (`shop_social_sidebartwo_item_image_{1..4}.png`) nên **không kiểm được**. `.haravan-cli_remote.json` khớp 241 khoá — nói về **khoá**, không phải hash nội dung.
...
- **Đối chiếu byte với remote (đo lại 2026-09-13 sau khi làm mới toàn bộ)**: 143 tệp văn bản — độ dài byte bằng `size` API. Ảnh: CDN trả **đúng content-type** theo đuôi tệp cho cả 3 khoá đo (`image/jpeg`, `image/png`); gửi `Accept` không đổi câu trả lời ⇒ không có byte sai định dạng, và parity byte với bản gốc là **không đạt được** với 2 khoá đang giữ bản chuyển đổi.
- **Byte của ảnh nhị phân không phải tiêu chí ổn định**: cùng một `public_url`, CDN trả kết quả khác nhau ở các thời điểm (`hera_index_hero_1_mobile.jpg`: 45.507 B rồi 55.681 B) và theo `Accept` (`Vary: accept, accept-encoding`; `hera_index_hero_2_mobile.jpg` với `Accept: image/webp` → 76.840 B webp, không gửi `Accept` → 98.904 B jpeg). `size` là trường **không diễn giải được** (không khớp ổn định với `attachment` lẫn byte CDN: 55.681/45.507, 2.140/1.653, 1.542/1.288); nội dung uỷ nhiệm là `value`/`attachment`. Trạng thái mirror hiện tại: **239/241 tệp khớp `size`**; **2 tệp mà client tải của script luôn nhận bản biến thể** (`hera_index_hero_1_mobile.jpg` 45.507 thay vì 55.681, `shop_coupon_item_image_2.png` 1.653 thay vì 2.140) trong khi `size`/`public_url` mô tả bản gốc — cùng URL, client GET trần nhận bản gốc, nên **byte parity phụ thuộc client và không phải tiêu chí**. Hai khoá đó được đánh dấu `variantRepresentation: true` nên tín hiệu độ dài chỉ bắn một lần (lần đầu `refreshed 2`, sau đó `refreshed 0, skipped 241`). `layout/theme.liquid` đổi thật trên remote (stamp `17:44:45.982Z`) và đã đồng bộ (2.851 byte = `size`). Mọi ảnh có marker kết thúc hợp lệ (JPEG `ffd9` / PNG `IEND`) ⇒ **không tệp nào bị cắt**.
- **Theme đích bị ghi lại giữa phiên (18:03:06Z–18:10:21Z)**: toàn bộ 241 `updated_at` nằm trong khoảng đó, `?v` 9→101, nội dung lưu trữ bị mã hoá lại nhỏ hơn (`attachment` ≠ `size` ở 87 khoá), và **4 asset hỏng phía remote** — `assets/shop_social_sidebartwo_item_image_{1..4}.png` có `public_url` sai host (`https://cdn.hstatic.netmes/...`) và `attachment` rỗng. Đọc không làm đổi theme trong cửa sổ kiểm chứng (3 pha 60 s, 0 stamp đổi), CLI không push từ mirror (`_remote.json` nguyên mtime, không có `.hrv-sync-state.json`). Chi tiết + giả thuyết ở kickoff §9.
- **Stamp đổi giữa phiên**: `updated_at` của `hera_index_hero_2_mobile.jpg` `17:02:03Z → 17:35:29Z`, `shop_social_sidebar_item_image_1.png` → `17:35:42Z`, `shop_social_sidebar_item_image_5.png` → `17:35:47Z` (các asset khác vẫn `17:02:03Z`); ba khoá này nay khớp đúng bản gốc lưu trữ (98.904 / 1.542 / 5.091 = `size` = `fresh GET`). Không suy ra nguyên nhân chỉ từ stamp: `hera_index_hero_1_mobile.jpg` đổi byte CDN phục vụ (45.507 → 55.681) mà stamp không đổi. Script pull so `updated_at` (ảnh), độ dài byte (văn bản) và độ lệch `size` (ảnh, một lần rồi ghi `variantRepresentation`) nên tự tải lại đúng các khoá có stamp nhích; hai lần chạy liên tiếp sau đó: `fetched 0, refreshed 0, skipped 241`.
