...
- **Byte của ảnh nhị phân không phải tiêu chí ổn định**: cùng một `public_url`, CDN trả kết quả khác nhau ở các thời điểm (`hera_index_hero_1_mobile.jpg`: 45.507 B rồi 55.681 B) và theo `Accept` (`Vary: accept, accept-encoding`; `hera_index_hero_2_mobile.jpg` với `Accept: image/webp` → 76.840 B webp, không gửi `Accept` → 98.904 B jpeg). `size` API là **kích thước bản gốc lưu trữ**; thứ CDN trả có thể là bản biến thể. Trạng thái mirror hiện tại: **239/241 tệp khớp `size`**, còn **2 tệp giữ biến thể cũ** (`hera_index_hero_1_mobile.jpg` 45.507 thay vì 55.681, `shop_coupon_item_image_2.png` 1.653 thay vì 2.140) trong khi `public_url` hiện trả đúng bản gốc — tức bên lệch là **mirror**, không phải CDN; hai tệp này chỉ khớp lại khi chạy `HARAVAN_FORCE_REFRESH=1` (stamp của chúng chưa từng nhích nên lần chạy thường bỏ qua). Mọi ảnh có marker kết thúc hợp lệ (JPEG `ffd9` / PNG `IEND`) ⇒ **không tệp nào bị cắt**.
