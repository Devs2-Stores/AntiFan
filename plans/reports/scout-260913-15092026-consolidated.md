...
- **Byte của ảnh nhị phân không phải tiêu chí ổn định**: cùng một `public_url`, CDN trả kết quả khác nhau ở các thời điểm (`hera_index_hero_1_mobile.jpg`: 45.507 B rồi 55.681 B) và theo `Accept` (`Vary: accept, accept-encoding`; `hera_index_hero_2_mobile.jpg` với `Accept: image/webp` → 76.840 B webp, không gửi `Accept` → 98.904 B jpeg). `size` API là **kích thước bản gốc lưu trữ**; thứ CDN trả có thể là bản biến thể. Trạng thái mirror hiện tại: **239/241 tệp khớp `size`**; **2 tệp mà client tải của script luôn nhận bản biến thể** (`hera_index_hero_1_mobile.jpg` 45.507 thay vì 55.681, `shop_coupon_item_image_2.png` 1.653 thay vì 2.140) trong khi `size`/`public_url` mô tả bản gốc — cùng URL, client GET trần nhận bản gốc, nên **byte parity phụ thuộc client và không phải tiêu chí**. Hai khoá đó được đánh dấu `variantRepresentation: true` nên tín hiệu độ dài chỉ bắn một lần (lần đầu `refreshed 2`, sau đó `refreshed 0, skipped 241`). `layout/theme.liquid` đổi thật trên remote (stamp `17:44:45.982Z`) và đã đồng bộ (2.851 byte = `size`). Mọi ảnh có marker kết thúc hợp lệ (JPEG `ffd9` / PNG `IEND`) ⇒ **không tệp nào bị cắt**.
