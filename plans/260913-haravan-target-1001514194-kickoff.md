...
- **Độ trung thực của bản pull (đo 2026-09-13)**: 241/241 khoá khớp; 0 thiếu, 0 thừa. **143 tệp văn bản**: 0 tệp lệch kích thước so với `size` remote; mẫu sha256 khớp. **5 ảnh nhị phân** có `size` API khai **lớn hơn** số byte CDN phục vụ (`hera_index_hero_1_mobile.jpg` 55.681 vs 45.507; `hera_index_hero_2_mobile.jpg` 120.262 vs 98.904; `shop_coupon_item_image_2.png` 2.140 vs 1.653; `shop_social_sidebar_item_image_1.png` 1.542 vs 1.288; `shop_social_sidebar_item_image_5.png` 5.091 vs 4.736) — nghĩa là `size` là kích thước **bản gốc lưu trữ**, còn CDN phục vụ bản biến thể. Bản local **khớp byte với CDN cho cả 5 tệp**: GET `public_url` 3 lần mỗi tệp → cùng độ dài + cùng sha256 bằng bản local (ví dụ hero_1 `cd5d8c93…`), và một bản pull mới vào thư mục tạm cho ra đúng các hash đó. Tất cả 5 tệp đều có marker kết thúc hợp lệ (JPEG `ffd9`, PNG `IEND`) ⇒ **không có tệp nào bị cắt**. Ghi chú: cùng một URL `?v=3` đã trả 55.681 byte ở lần đo đầu và 45.507 byte ở các lần sau (ổn định 3/3) ⇒ **đừng khoá giả định theo tham số `?v=`**, và **đừng dùng `size` API làm tiêu chí đủ/thiếu cho ảnh**.
- **Rác còn lại của CLI pull**: `15092026/.hrv_tmp_pull-<pid>-<ts>/assets/` gồm đúng 6 tệp — 4 bản khác byte với bản trong `assets/` (`hera_index_hero_1_mobile.jpg` 55.681, `hera_index_hero_2_mobile.jpg` 120.262, `shop_coupon_item_image_2.png` 2.140, `shop_social_sidebar_item_image_1.png` 1.542, tức các bản **gốc lưu trữ**), 1 tệp **0 byte** (`shop_social_sidebar_item_image_5.png`, dấu vết tải dở), và 1 tệp **trùng khít** bản trong `assets/` (`share_fb_home.png`). Không phải nội dung theme; xoá được. (Chỉ 5 khoá nằm trong nhóm `size` lệch — không có tệp thứ sáu nào cần đi tìm.)
