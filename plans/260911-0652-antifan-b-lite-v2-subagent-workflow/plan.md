...
## Status (2026-09-11, after execution rounds 1-8)
...
| 5 Supervised fix proof | **done (target withdrawn on measurement)** | `04ef707`; the pinned page-02 @390 `deltaGeometry 10` does not reproduce across five independent designs (paired capture 4270/170 both sides and 0 of 458 header nodes differing; 6 loads; an 84-sample 25s series under measured mobile emulation; a viewport sequence; the campaign's own desktop-then-mobile document order), while the live reference reproduces the campaign's recorded 4270 exactly - so the clone has no defect to fix and no `packages/site-clone` edit was made. The loop's refusal and merge paths were proven on a real tree in both directions, and a real supervised round on a pipeline-consumed subject (`scripts/lib/campaign-verdicts.mjs`, receipts committed under `.canary/staging/fixround-20260911-154329/`) ended `FIXED_VERIFIED`: audit exit 0, merge exit 0 with `postMergeVerification.verified`, confirmation suite 14/14 from a measured 13/1. Residual gap recorded, not papered over: a structural refusal was published from an unreproducible read, and requiring two consecutive identical structural reads before minting one (else `INCONCLUSIVE`) is an open owner decision |
...
- [x] Typed outcomes with evidence paths were measured (`REFUSED_TOUCHED_PATH`, `REFUSED_SCOPE_EXPANSION`, `OK`, and `FIXED_VERIFIED` on the verdict-reader subject whose confirmation is the route-identity gate suite passing 14/14 after a measured 13/1 reproduction), and a self-verification claim is a refusal class, not an accepted result. On the pinned clone surface no fix was verifiable because the delta was measured away, and the phase's own risk clause admits a typed audited outcome as a pass.
