...
- [x] A Hub tab shows, per recorded dispatch name: call count, terminal state mix, `error.code` histogram, latency p50/p95 — every number traceable to an admitted ledger line.
...
- [x] Reconciliation invariants print and hold **separately**, in their **general** forms: key-level `classifiedKeys + unattributedKeys == compositeKeys`, and frame-level `frames == compositeKeys + superseded + keylessFrames`. A single invariant mixing frame counts with key counts is false by construction whenever `superseded > 0` (independent passes measured 19,096 / 19,164 / 19,215 admitted frames against 18,878 / 18,862 / 18,884 distinct composites). The exact-looking pair `classified + unattributed == compositeKeys` and `frames == composites + superseded` is only valid in the `keylessFrames === 0` case, so it is stated as that special case and never as the invariant; `keylessFrames` is expected to be 0 live and is printed anyway so a non-zero value cannot hide.
- [x] Latency is computed only from non-excluded frames; the excluded population is reported with **its own p50/p95** (measured: excluded p50 262,957 ms / p95 2,427,228 ms vs headline p50 13 ms / p95 3,787-3,799 ms). `interrupted` frames are **kept**.
- [x] Empty/unbound data root renders `UNMEASURED`/`NO_DATA` naming the mechanism — never `0`, never a fabricated row; the CLI has no repo-relative fallback. The **absolute** store path is printed by the CLI and recorded in the on-disk receipt; the Hub payload carries only a display label (constraint D), so "the resolved path" is disclosed where it is safe and nowhere else.
- [x] Every row states its retained window (`firstSeen`/`lastSeen`) and the payload states the retention horizon; when any short-read occurred, rows are marked `lowerBound: true`, and a content change at equal size/mtime renders `UNMEASURED` with no rows (fail closed).
- [x] A quarantine margin is reported as a **first-class labelled line** (frames present, frames admitted, frames named-invalid, why) — never folded into the admitted total and never presented as a recovery promise.
- [x] `getState()` returns within its declared budget or returns `UNMEASURED`/`READ_BUDGET_EXCEEDED`; a main-thread pass is impossible by construction (worker thread) and the CLI proves the reader is electron-free.
- [x] The IPC payload contains **no** raw frame and **no** `runtimeLeaseToken` / `authoritySnapshot` / `requestId` / `paramDigest` / absolute path — **in no value and in no object key**; every histogram key is a vocabulary token or `UNRECOGNIZED`; the handler is sender-gated and returns a well-formed `UNMEASURED` on refusal.
- [x] Every string rendered by the new tab is escaped; a fixture frame whose `name` and `error.code` contain markup produces no element and no attribute.
- [x] `scripts/check-mcp-dispatch-payload.mjs` reads **source and a committed fixture only** (never `invocations/`, never `.antifan/telemetry/`), fails on a seeded payload that carries a frame field, and cannot fail a launch for data reasons.
- [x] `core.*` renders either as measured proxy attempts (Phase 5 landed, `omp-proxy` provenance, `proxy-attempt` unit — and the CLI's `core-attempts` section reads the same store) or as an explicit labelled hole naming the mechanism and every launch path that cannot carry the environment variable.
...
- [x] No new advertised MCP tool exists; `npm run compile` gates stay green.
- [x] The CLI alone proves determinism: two runs on one frozen copy are byte-identical over every store-derived value — i.e. modulo `asOf` and `census.limits.elapsedMs` (the pass's own wall-clock self-measurement, which can never be deterministic and must be **named**, never silently normalized away). The Hub makes no byte-identity promise.

**Integration verification status (integrator, after the final compile and the commit that carries it).** Fourteen of the fifteen criteria above are marked from executed evidence rather than from intent: this plan's own suite (exit 0, every test passing), `npm run compile` exit 0 over the full eight-step chain, `npm run audit` exit 0, `plans:check` exit 0, an independent adversarial review that reproduced ten claims against the delivered tree, and the acceptance receipt's executed rows. Two things are recorded as **not** proven, and neither is smoothed over:

- **Criterion 148, the `certify:core-freeze` re-certification, is NOT run.** `scripts/certify-core-freeze.cjs` unlinks the existing `freeze-certificate.json` and the three `windows-soak-run-*.json` files before it re-runs the whole compile and three Electron soak passes, so running it against a tree that is still moving would destroy another plan's certificate and replace it with one bound to a revision no commit reproduces. That certificate's build identity was itself blind to this change: it hashed `invocation-ledger.js` but not the extracted `invocation-frame-checksum.js`, so the only ledger behaviour this work altered could not move the identity at all. That list is now corrected, and the re-certification is left for a quiet tree.
- **The test lanes are not green as a set, and none of the failures is attributable to this change.** Across four runs the failing tests were `goal-runner.test.mjs:94` ("loses at most one unit of work when killed mid-phase", a 15 s budget), `bridge-pairing-queue-concurrency` ("queue with two live codes", a 30 s `waitFor`) and `first-party-network-tracker` ("does NOT block quiescence on never-ending third-party analytics beacons"). Each passes when its own file is run alone, none of those files is touched by this change, and `npm run test:unit` returned 1111 tests / 0 fail on the run immediately after one where it had failed. They are recorded as unattributed load-sensitive failures, not as a known-bad list: the set changes between runs, which is itself the evidence that the cause is load rather than a regression.
