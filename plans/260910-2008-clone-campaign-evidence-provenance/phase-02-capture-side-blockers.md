...
7. **Mobile bundle.** At the 390 pass, dump the mobile reference (`reference/reference-mobile.html`) and build `clone/mobile/` from it, mirroring `canary-run.mjs:359`; wire the 390 case to the mobile bundle and give a missing bundle a typed declared exception. Record the measured geometry of both sides per case.
