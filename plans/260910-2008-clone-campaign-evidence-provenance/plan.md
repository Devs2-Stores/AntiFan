...
- Every mutating bridge call runs against the pinned isolated instance, never the user's instance on 20130. Single-writer state holds everywhere: the launcher owns `.canary/state/canary-instance.json`, the run owns `.canary/15-pages/.campaign.lock` (exclusive; released on controlled exits, reclaimed when its recorded holder is dead; a live holder exits non-zero with `RUN_IN_PROGRESS`), a page's attempt directory is written once and never rewritten, and per-page evidence and the run aggregate are published by atomic replace so a partial write never becomes the published view.
...
- [ ] Concurrent invocation cannot corrupt evidence: a live run lock refuses a second invocation with `RUN_IN_PROGRESS` while a crashed holder's lock is reclaimed, each page's attempts are immutable directories with their own evidence, and the published evidence and report change only by atomic replace.
