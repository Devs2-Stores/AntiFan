...
- [ ] Concurrent invocation cannot corrupt evidence: a live run lock refuses a second invocation with `RUN_IN_PROGRESS` while a crashed holder's lock is reclaimed only after it is proven dead, each attempt is an immutable directory carrying its own evidence and both viewport bundles, and the published view changes only through pointer files rewritten temp-file → `rename`.
