...
Facts 1–4 are the live blocker; facts 5–7 and 10–13 are trust and hygiene defects; only fact 12 is a fidelity difference, and it is stale. The campaign has been failing to measure, not failing to match.
...
| 2 | Every verdict names the bundle (sha256 + bytes) and the instance (pid, run, attempt) that produced it; a verdict against a superseded or unidentified target cannot be written, and a blocked run exits non-zero | P0 |
...
- Do not chase the 5 known 1440 structural deltas until the measurement path yields real numbers against the current bundle.
...
- [ ] Every case carries verdict, cause code, bundle sha256 + bytes, instance pid, run/attempt ids, and both sides' measured capture geometry.
- [ ] A verdict whose bundle hash differs from its generation telemetry fails the run instead of being written.
...
- [ ] The runner exits non-zero whenever it produces fewer cases than requested or any FAIL.
...
1. **Settle predicate.** The live storefront mutates nodes every ~1 s while geometry and metrics are provably stable. Recommended: make the settle proof the *cross-pass equality of the structural metrics it already computes* (section count, card count, document height, and the per-phase flags), which is the invariant the contract actually needs, with the geometry signature as corroboration only. `SETTLE_VISUAL_EXPR` truncates to the first 4000 elements and samples ~800 ms (`.canary/tools/canary-settle.mjs:75`), and evaluating layout signatures inside a MutationObserver risks synchronous layout thrashing, so neither is a good primary mechanism. The two-consecutive-pass rule stays, and every failing predicate is persisted. Alternative: keep the `childList` predicate and add a bounded cosmetic-churn exemption with the churn count recorded in the verdict.
...
6. **Clearing the ten orphaned tabs.** They do not block runs (fact 13), so clearance is a tab-economy action, not a prerequisite gate. Recommended: the owner closes them in the isolated instance's window; deleting that instance's persisted state is an alternative that needs explicit approval because it lies outside this repository. An automated privileged cleanup would be a product change and is out of scope here; session scoping must not be weakened to make it convenient.
