...
- Every mutating bridge call runs against the pinned isolated instance, never the user's instance on 20130. Single-writer state holds everywhere: the launcher owns `.canary/state/canary-instance.json`, the run owns `.canary/15-pages/.campaign.lock` (acquired before any mutation; released in the top-level `finally`; reclaimed only after the process-identity helper proves the holder dead — pid gone, or that pid now carrying a different start time, never file age; a live holder exits non-zero with `RUN_IN_PROGRESS`), a page's attempt directory is written once and never rewritten, and published views change only through pointer files (`<page>/current-attempt.json`, `.canary/15-pages/current-report.json`) rewritten temp-file → `fsync` → `rename`, so a partial write never becomes the published view.
...
Phase 2's own file carries the findings that were discovered while executing it — the
per-pass guard registry, the pins that manufactured mismatches, the bundle-unbinding
extractor, the residue that reached published bundles, and the mint that reached a bridge
which reset it. Add them to this table only if the next executor needs them here.
...
- [ ] Concurrent invocation cannot corrupt evidence: a live run lock refuses a second invocation with `RUN_IN_PROGRESS`, while a dead holder's lock is reclaimed only when the process-identity check proves it dead — the pid is gone, or that pid now carries a different start time; each attempt is an immutable directory carrying its own evidence and both viewport bundles, and the published view changes only through pointer files rewritten temp-file → `rename`.
