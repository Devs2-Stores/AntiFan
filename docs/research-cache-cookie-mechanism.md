...
7. **Chấp nhận, không sửa bằng code** — cookie xoay vòng của Google (`*PSIDTS`/`*PSIDRTS`) là single-owner; copy sang client thứ hai thì hai client đá nhau. Chiến lược chốt: coi AntiFan là owner riêng (đăng nhập một lần trong app) hoặc dùng vault như bản sao một chiều, không share vòng xoay. **Cập nhật 2026-09-22:** cùng nguyên lý này lặp lại ở Haravan và nay đã được **thực thi bằng code** trên kênh Companion — xem P3.
...
### P3 — ranh giới danh tính trên kênh Companion (2026-09-22)

**Triệu chứng người dùng:** "Cache/Cookie/Password AntiFan đang quản lý kiểu gì mà mọi 20-30p đã mất và bắt Login lại" — phiên Haravan admin.

11. **Đã, và lần này sửa bằng code** — Companion đẩy đè cookie danh tính. Đo được: Chrome thật đang giữ `accounts.haravan.com/idsrv` (1498 B, hạn ~2026-10) và `idsv.device`; `SCOPE_PROFILES.ecommerce` khớp `haravan.com` theo suffix nên `accounts.haravan.com` **nằm trong scope**; `triggerAutoHydration()` chạy lại sau mỗi handshake MV3 (log 10–18 phút/lần) với `enabledProfiles` lấy từ default (`chrome.storage.local` không có override nào) và `syncActiveTabOnly = false`. Jar `persist:profile-profile-2` của AntiFan khi đó giữ `idsrv.session=6F7B009FD3A3CA4F6B9BB894D9D47747`. Cặp `idsrv`/`idsrv.session` là hai nửa của **một** phiên server-side, nên ghi `idsrv` của trình duyệt khác vào là tách cặp: IdentityServer huỷ phiên rồi trả về `/Account/Login?ReturnUrl=/connect/authorize/callback...` — đúng URL hai tab admin đang đứng lúc đo. Cơ chế trùng khít mục 7; khác biệt duy nhất là ở Haravan nó đang bị **thực thi bằng code** trên kênh extension thay vì chỉ nằm trong chiến lược.
12. **Đã** — `src/shared/identity-cookie-patterns.ts` là nguồn duy nhất: `IDENTITY_COOKIE_PATTERNS` + `isIdentityCookieName(name)`. Chốt ở **hai** đầu. Bên nhận `bridge-server.ts` lọc `rawCookies` **trước** mọi filter khác và cho **mọi** lớp credential — không chỉ grant của extension: nhánh master-token/attachment trước đây nhận `rawCookies` nguyên vẹn, không đi qua allowlist nào cả. Response thêm `authRejectedCount`, log `[antifan] cookie import refused … identity cookie(s)`. Bên gửi `domain-scoper.ts` trả `false` cho cookie danh tính **trước** cả nhánh wildcard `all`/`*` và nhánh active-tab, nên không cấu hình scope nào opt-in lại được.
13. **Phạm vi có chủ ý** — chỉ cookie **danh tính**: `idsrv`/`idsv.*` (IdentityServer4), `.AspNetCore.*` + `__RequestVerificationToken` (ASP.NET Core), `_secure_admin_session_id`/`sapo_admin_session` (phiên admin Sapo/Shopify), và token **xoay vòng** `*PSIDTS`/`*PSIDRTS`/`*PSIDCC` (đúng mục 7). Cookie **đăng nhập tĩnh** của Google (`SID`/`HSID`/`SSID`/`APISID`/`SAPISID`/`LOGIN_INFO`) **không** nằm trong danh sách: P0#4 đã cố ý mở allowlist cho nhóm Google, và mục 7 chỉ cấm **chia sẻ vòng xoay**, không cấm cookie tĩnh. Context storefront (cart, theme preview, `_haravan_session`, `_ab`/`shop_ref`/`_ga`) giữ nguyên đường đồng bộ.
14. **Bằng chứng** — `npm run compile` exit 0 (sinh lại `extension/background.js` 189 227 B, sync sang `%LOCALAPPDATA%\AntiFan\extension`). `extension-companion-pipeline` **12/12**. Đột biến để chứng minh test thật sự bắt lỗi chứ không xanh do không chạy: thay filter bên nhận bằng `false` → test ranh giới **fail** (`importedCount` 4 ≠ 2, tức cookie danh tính lọt vào store); thay filter bên gửi bằng `false` → test scope **fail**. Chạy luật bên gửi trên **30 cookie Haravan thật** đọc từ SQLite của Chrome: `accounts.haravan.com/idsrv` + `idsv.device` bị chặn, 25 cookie storefront còn sync (3 cookie `.haravan.app/_ga*` bị loại là do luật domain sẵn có — `haravan.app` ≠ `haravan.com` — không phải filter mới).
15. **Chưa kiểm chứng trên tiến trình đang chạy** — bản này **chưa load** vào app đang mở: code Main nằm trong tiến trình Electron đã khởi động trước khi sửa, và extension trong Chrome cũng chưa reload. Phép thử sống (POST `idsrv` vào bridge của app đang chạy) **cố ý không chạy**: tiến trình đang chạy là bản **cũ**, nó sẽ **ghi** cookie đó và phá phiên Haravan thật của người dùng — đúng cái lỗi đang sửa. Cần restart AntiFan + reload extension rồi mới đo lại.
