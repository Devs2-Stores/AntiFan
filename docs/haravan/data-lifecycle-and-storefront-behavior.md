...
| Dynamic variant changes update URL parameter `?variant={id}` via history replacement. | `UNVERIFIED` | `AntiFan/themes/phukienmaymoc-copy/assets/scripts.js.liquid:240–310` (2026-09-12) — source deleted 2026-09-13 with the old-target mirror. | None. Standard pattern. | Re-measure on the theme under work (local mirror of theme `1001514194`) or on any storefront's variant switcher. |
