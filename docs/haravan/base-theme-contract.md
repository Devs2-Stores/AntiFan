...
- `snippets/swatch.liquid`: Phân tích `product.options`, render mỗi giá trị thành một liên kết `/products/{handle}?variant={id}`. Đích của mỗi giá trị được phân giải qua 3 lượt so với biến thể đang render: (1) biến thể khớp giá trị đó ở **mọi tuỳ chọn còn lại** — lượt này cố ý **không xét tồn kho**, nên tổ hợp hết hàng vẫn được trỏ đúng biến thể của nó và gắn nhãn `(Hết hàng)` thay vì bị thay bằng giá trị khác; (2) một biến thể còn hàng mang giá trị đó, chỉ tới khi tổ hợp nói trên không tồn tại; (3) biến thể bất kỳ mang giá trị đó. Nhờ vậy server render đúng biến thể được chọn — không cần JS và không có giá trị giá/ảnh lệch pha.
