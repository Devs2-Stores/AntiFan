...
| `/collections` | `templates/list-collections.liquid` | `collections` | `?page={n}` |
...
| The collections index route `/collections` is served by `templates/list-collections.liquid` (not `templates/collection.liquid`). | `VERIFIED` | Live storefront `GET https://phukienmaymoc.com/collections` returns HTTP 200 with container class `template-list-collections` (2026-09-12); corpus census `templates/list-collections.liquid` in 41/45 measured roots, `templates/collections.liquid` 0/45. | Base-theme contract previously mapped the route to `templates/collection.liquid`. | None. |
| `{% form 'customer' %}` is the corpus-standard newsletter mechanism, although the form type is absent from the platform documentation. | `OBSERVED` | Corpus census (2026-09-12): 79 `.liquid` files across 40/45 measured roots use `{% form 'customer' %}`; only 2 files POST raw to `/account/contact` and 12 to `/contact`. | The documented form-type list (contact, customer_login, create_customer, reset_customer_password, activate_customer_password) does not include it. | Submit once against the unpublished staging theme and observe where the address lands (UNRESOLVED_06). |
