...
function evaluateMarkup(html: string, platform: EcommercePlatform): HsEvaluationResult {
  ...
  if (/deleteAddress/i.test(html) && !/(?:function\s+deleteAddress|(?:window\.)?deleteAddress\s*=)/i.test(html)) violations.push(makeViolation(platform, 'HS-04', 'Customer Address Deletion Handler', 'error', 'Customer address deletion references deleteAddress but no handler is defined', 'Define deleteAddress or use the platform-supported address deletion API', '[onclick*="deleteAddress"]'));
