...
function evaluateMarkup(html: string, platform: EcommercePlatform): HsEvaluationResult {
  ...
  for (const form of forms) {
    ...
    if (platform === 'sapo' && /comment/i.test(action) && /\bname\s*=\s*["'](?:author|comment\[author\]|email|comment\[email\]|body|comment\[body\])["']/.test(form)) violations.push(makeViolation(platform, 'HS-03', 'Sapo Comment Field Casing', 'warning', 'Sapo blog comment form uses lowercase field names instead of Author, Email, Body', 'Use capitalized Author, Email, Body field names', 'form[action*="comment"]', compact(form)));
  }
  ...
  const cdn = platform === 'haravan' ? /https?:\/\/[^"'\s>]*hstatic\.net\//i : platform === 'sapo' ? /https?:\/\/[^"'\s>]*(?:bizweb\.dktcdn\.net|dktcdn\.net|cdn\.sapo\.vn)\//i : /https?:\/\/[^"'\s>]*(?:cdn\.shopify\.com|shopifycdn\.com)\//i;
...
export class HsGateRules {
  public static getBrowserEvaluationScript(platform: EcommercePlatform): string {
    return `(() => {
      ...
      const isPlatformCdn = (srcUrl) => { try { const u = new URL(srcUrl, location.href); const h = u.hostname.toLowerCase().replace(/^www\\./, ''); if (platform === 'haravan') return h === 'hstatic.net' || h.endsWith('.hstatic.net'); if (platform === 'sapo') return h === 'dktcdn.net' || h.endsWith('.dktcdn.net') || h === 'cdn.sapo.vn' || h.endsWith('.cdn.sapo.vn'); if (platform === 'shopify') return h === 'shopifycdn.com' || h.endsWith('.shopifycdn.com') || h === 'cdn.shopify.com'; return true; } catch { return false; } };
      const allImages = Array.from(document.querySelectorAll('img')); const images = allImages.filter((img) => /featured|product/i.test(img.className || '') || allImages.length === 1);
      for (const image of images) { const src = image.getAttribute('src') || ''; if (src && (!/^https?:\\/\\//i.test(src) || !isPlatformCdn(src))) { add('HS-05', 'Featured Image CDN URL', 'warning', 'Featured image does not use an absolute platform CDN URL', 'Use an absolute URL served from the platform CDN', 'img'); break; } }
