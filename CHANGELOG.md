...
- Swatch: chọn một thuộc tính không còn lật ngược thuộc tính khác. Mỗi giá trị chọn biến thể theo 3 lượt — (1) biến thể còn hàng và khớp mọi thuộc tính còn lại với biến thể đang render, (2) biến thể còn hàng mang giá trị đó, (3) biến thể bất kỳ mang giá trị đó — nên trên sản phẩm 2 thuộc tính, đang ở Xanh/S bấm M sẽ sang Xanh/M thay vì Đỏ/M. Đã render thật qua LiquidJS: 17/17 kiểm tra.
- Bổ sung `templates/list-collections.liquid` cho route `/collections` (trước đây route này rơi vào `templates/collection.liquid` và chỉ ra trạng thái rỗng). Route được xác minh: storefront thật trả HTTP 200 với class `template-list-collections`, và 41/45 root corpus có file này.
