...
### Sửa lỗi — App tự văng khi chụp tab agent offscreen (`Page.captureScreenshot fromSurface:false`)
- **Nguyên nhân**: tab agent Dual-Plane là OSR (`security-policy.ts` set `prefs.offscreen = true`, `paintWhenInitiallyHidden = false`), nên **không có native window**. Lệnh `Page.captureScreenshot { fromSurface: false }` là đường native-window snapshot của Chromium → `WindowSnapshotReachedScreen` → `GrabNativeWindowSnapshot(GetNativeView() = null)` → **deref NULL, chết cả browser process** (app biến mất, không log JS nào). Ba dump của app (`runtime/crashDumps/reports/`: 16:11:59, 16:19:47, 16:38:46) đều là `0xC0000005` tại địa chỉ `0x0`.
- **Sửa**: `tab-devtools-host.ts` không bao giờ gửi `fromSurface:false` nữa — Tier 2 (fallback CDP) và `captureVerificationScreenshot` đều dùng `fromSurface:true`; **xoá hẳn Tier 3 "Offscreen Native View Paint Fallback"** (trùng Tier 2, chỉ khác đúng đường native gây chết). Đích không có compositor surface giờ trả lỗi có kiểu (`CAPTURE_EMPTY_PAYLOAD` / CDP error) thay vì giết tiến trình.
- **Kiểm chứng**: harness đo trực tiếp — `WebContentsView{offscreen:true}` đã attach + `fromSurface:false` → tiến trình chết (exit 5, dump `0xC0000005 @ 0x0`); cùng view đó với `fromSurface:true` → 3.836 B trong 43 ms. `test/main/playwright-parity-kernel.test.ts` ca 6 nay khẳng định mọi `Page.captureScreenshot` phải `fromSurface:true` (regression gate chống tái phát), `tab-devtools-host.test.ts` ca 13/15 cập nhật theo contract mới. `npm run test:main`: 1189 pass / 0 fail (2 skipped).
- **Giới hạn đã đo**: tab offscreen OSR khi không attach thì không rasterize được (`capturePage` trả rỗng, CDP surface treo); attach thì `capturePage` trả pixel của cửa sổ (surface bleed), không phải nội dung tab. Muốn agent chụp được tab offscreen thật cần host window ẩn riêng — chưa làm trong bản này.
