...
describe('Theme QA vertical slice', () => {
  it('inspects, edits, reloads, and emits bounded durable evidence for one exact target', async () => {
    ...
    let reloads = 0;
    const browser = new BrowserControlPort({ getTabList: () => [{ id: 'tab-1' }], navigate: () => true, reload: () => { reloads++; return true; }, getDom: async () => '<main>fixture</main>', captureScreenshot: async () => Buffer.from('png').toString('base64'), evalJs: async () => null }, new ArtifactStore({ root: path.join(root, 'artifacts') }));
