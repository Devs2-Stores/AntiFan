...
const definitions = [
  ...
  ['core.context_pack', 'Build a Context Pack for a task: relevant claims, unresolved conflicts, unknowns, permission scope. Packs dedupe on (task, platform, sessionId, scope).', { task: { type: 'string' }, platform: { type: 'string' }, unitIds: { type: 'array', items: { type: 'string' } }, limit: { type: 'number' }, sessionId: { type: 'string' }, includeGlobal: { type: 'boolean' }, scope: { type: 'string', description: 'Project root (absolute path) whose units rank first. A preference, not a filter: cross-project claims still fill the pack.' } }, ['task']],
  ['core.recommend', 'Recommend from evidence: Context Pack + recommendation or explicit abstention.', { task: { type: 'string' }, platform: { type: 'string' }, unitIds: { type: 'array', items: { type: 'string' } }, sessionId: { type: 'string' }, includeGlobal: { type: 'boolean' }, scope: { type: 'string', description: 'Project root (absolute path) whose units rank first. A preference, not a filter: cross-project claims still fill the pack.' } }, ['task']],
  ...
  ['core.context_pack_v2', 'Build an enriched Context Pack: claims + rules + historical cases + pitfalls + workarounds + recommended pattern + uncertainty + confidence.', { task: { type: 'string' }, platform: { type: 'string' }, unitIds: { type: 'array', items: { type: 'string' } }, limit: { type: 'number' }, sessionId: { type: 'string' }, includeGlobal: { type: 'boolean' }, scope: { type: 'string', description: 'Project root (absolute path) whose units rank first. A preference, not a filter: cross-project claims still fill the pack.' } }, ['task']],
